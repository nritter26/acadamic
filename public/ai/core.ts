(function (root: Record<string, unknown>) {
  ...
  type CoursePhase = Record<string, CourseItem>;
  type CourseLang = Record<string, CoursePhase>;

  // ── Globals (set by other scripts) ──
  const w = root as Record<string, unknown>;
  const aiTutorResponses: TutorEntry[] = (w.aiTutorResponses as TutorEntry[]) || [];
  const courseData = (): Record<string, CourseLang> => (w.courseData as Record<string, CourseLang>) || {};
  const currentLang: () => string = () => (w.currentLang as string) || '';
  const currentTopic: () => string = () => (w.currentTopic as string) || '';
  const currentPhase: () => string = () => (w.currentPhase as string) || '';
  ...
  const LANG_NAMES: Record<string, string> = {
    ...
    cloud: 'Cloud', k8s: 'Kubernetes', terraform: 'Terraform',
    react: 'React', vue: 'Vue', angular: 'Angular', svelte: 'Svelte',
    next: 'Next.js', nuxt: 'Nuxt', sveltekit: 'SvelteKit', remix: 'Remix',
    vite: 'Vite', webpack: 'Webpack', tailwind: 'Tailwind', bootstrap: 'Bootstrap',
    node: 'Node.js', express: 'Express', fastapi: 'FastAPI', flask: 'Flask',
    django: 'Django', spring: 'Spring', graphql: 'GraphQL', prisma: 'Prisma',
    redis: 'Redis', reactnative: 'React Native', flutter: 'Flutter',
    cypress: 'Cypress', playwright: 'Playwright', sql: 'SQL',
  ...
  // ── ERROR_PATTERNS ──
  const ERROR_PATTERNS: ErrPattern[] = [
  ...
  // ── KEYWORD_ISSUES (static code review patterns) ──
  const KEYWORD_ISSUES: KeywordIssue[] = [
  ...
  // ── Helper: word-boundary keyword test ──
  ...
  // ── detectLanguageInQuery ──
  function detectLanguageInQuery(q: string): string | null {
  ...
  // ── getLanguageIntro ──
  function getLanguageIntro(langCode: string): { displayName: string; exp: string; code: string } | null {
  ...
  // ── getAIResponse (enhanced — word-boundary + scoring) ──
  function getAIResponse(input: string, history?: unknown[]): string {
    const q = input.toLowerCase().trim();
    if (!q) return "Ask me something about programming!";

    // Follow-up detection: conversation-aware
    if (history && history.length >= 2) {
      const hist = history as Array<{ role: string; text: string }>;
    ...
    // Combined keyword matching (fallback)
    for (const entry of aiTutorResponses) {
  ...
  // ── getLocalAIResponse ──
  function getLocalAIResponse(input: string): string | null {
    const q = input.toLowerCase().trim();
    ...
    const data = courseData();
    const langData: CourseLang | undefined = data[searchLang];
    if (!langData) return null;

    // Asking about different language
    ...
    // Synonym-expanded curriculum search
    ...
    let best: { phase: string; topic: string; code: string; exp: string; lang: string } | null = null;
    let bestScore = 0;
  ...
  // ── getErrorTutorTip ──
  function getErrorTutorTip(topic: string, output: string): string | null {
    ...
    const topicTips: Record<string, string> = {
  ...
  // ── analyzeUserCodeClient ──
  function analyzeUserCodeClient(code: string, lang: string): string {
  ...
  // ── localFindLineIndex ──
  function localFindLineIndex(text: string, matchIndex: number): number {
    const before = text.slice(0, matchIndex);
    return before.split('\n').length - 1;
  }

  // ── localAnalyzeStructure ──
  function localAnalyzeStructure(code: string): CodeIssue[] {
  ...
  // ── localCheckKeywords ──
  function localCheckKeywords(code: string, lang: string): CodeIssue[] {
    const issues: CodeIssue[] = [];
    const lines = code.split('\n');
  ...
  // ── localLanguageReviewIssues ──
  function localLanguageReviewIssues(code: string, lang: string): CodeIssue[] {
    const issues: CodeIssue[] = [];
    const lines = code.split('\n');
  ...
  // ── localCalculateScore ──
  function localCalculateScore(issues: CodeIssue[], lineCount: number): number {
    let score = 10;
  ...
  // ── localCodeReview ──
  function localCodeReview(code: string, lang: string): CodeReviewResult {
    if (!code || !code.trim()) {
    ...
    if (allIssues.length > 0) {
      review += '\n\n**Issues Found:**\n';
      const bySeverity: Record<string, CodeIssue[]> = { error: [], warning: [], style: [], info: [] };
  ...
  // ── localCodeExplain ──
  function localCodeExplain(code: string, lang: string, topic: string): CodeExplainResult {
    if (!code || !code.trim()) {
    ...
    const varDecls: { name: string; val: string }[] = [];
  ...
  // ── localGenerateExercise ──
  function localGenerateExercise(topic: string, lang: string, level: string): Exercise {
    const exercises: Record<string, Partial<Record<string, Exercise>>> = {
      variables: {
        beginner: {
