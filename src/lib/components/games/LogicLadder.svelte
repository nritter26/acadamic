...
<div class="game-ll" class:transitioning={ctrl.transitioning}>
  <div class="g-top">
    <span class="g-badge">logic-ladder</span>
  ...
  <div class="ll-choices">
    {#each (ctrl.challenge.choices || []) as choice}
      <button
        class="ll-choice"
        class:ll-correct={ctrl.feedbackType === 'ok' && choice === ctrl.challenge.answer}
        class:ll-wrong={ctrl.feedbackType === 'wrong' && selected === choice}
        onclick={() => pick(choice)}
...
<style>
  .game-ll { max-width: 700px; padding: 32px; border: 1px solid #334155; border-radius: 24px; background: linear-gradient(145deg, rgba(168,85,247,0.18), rgba(15,23,42,0.9)); position: relative; transition: opacity 0.15s; }
  ...
  .ll-choices { display: flex; flex-direction: column; gap: 8px; margin-bottom: 12px; }
  .ll-choice { padding: 14px; background: #0f172a; border: 2px solid #1e293b; border-radius: 10px; color: #e2e8f0; font-size: 14px; cursor: pointer; text-align: center; font-weight: 600; }
  .ll-choice:hover:not(:disabled) { border-color: #a855f7; }
  .ll-choice:disabled { cursor: default; }
  .ll-correct { border-color: #10b981 !important; background: rgba(16,185,129,0.08); }
  .ll-wrong { border-color: #ef4444 !important; background: rgba(239,68,68,0.08); }
