...
<div class="game-sql" class:transitioning={ctrl.transitioning}>
  <div class="g-top">
    <span class="g-badge">sql-join-match</span>
    <span class="g-progress">{ctrl.progress.current}/{ctrl.progress.total}</span>
  </div>
  <h1>🗄️ {game.title}</h1>
  <div class="g-prompt">{ctrl.challenge.prompt}</div>

  <div class="sql-choices">
    {#each (ctrl.challenge.choices || []) as choice}
      <button
        class="sql-choice"
        class:sql-correct={ctrl.feedbackType === 'ok' && choice === ctrl.challenge.answer}
        class:sql-wrong={ctrl.feedbackType === 'wrong' && selected === choice}
        onclick={() => pick(choice)}
...
<style>
  .game-sql { max-width: 700px; padding: 32px; border: 1px solid #334155; border-radius: 24px; background: linear-gradient(145deg, rgba(59,130,246,0.18), rgba(15,23,42,0.9)); position: relative; transition: opacity 0.15s; }
  ...
  .sql-choices { display: flex; flex-direction: column; gap: 8px; margin-bottom: 12px; }
  .sql-choice { padding: 14px; background: #0f172a; border: 2px solid #1e293b; border-radius: 10px; color: #e2e8f0; font-size: 14px; font-weight: 700; cursor: pointer; text-align: center; font-family: 'JetBrains Mono', monospace; }
  .sql-choice:hover:not(:disabled) { border-color: #3b82f6; }
  .sql-choice:disabled { cursor: default; }
  .sql-correct { border-color: #10b981 !important; background: rgba(16,185,129,0.08); }
  .sql-wrong { border-color: #ef4444 !important; background: rgba(239,68,68,0.08); }
