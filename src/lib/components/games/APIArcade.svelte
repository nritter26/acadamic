...
<div class="game-api" class:transitioning={ctrl.transitioning}>
  <div class="g-top">
    <span class="g-badge">api-arcade</span>
  ...
  <div class="api-choices">
    {#each (ctrl.challenge.choices || []) as choice}
      <button
        class="api-choice"
        class:api-correct={ctrl.feedbackType === 'ok' && choice === ctrl.challenge.answer}
        class:api-wrong={ctrl.feedbackType === 'wrong' && selected === choice}
        onclick={() => pick(choice)}
...
<style>
  .game-api { max-width: 700px; padding: 32px; border: 1px solid #334155; border-radius: 24px; background: linear-gradient(145deg, rgba(16,185,129,0.18), rgba(15,23,42,0.9)); position: relative; transition: opacity 0.15s; }
  ...
  .api-choices { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 12px; }
  .api-choice { padding: 14px; background: #0f172a; border: 2px solid #1e293b; border-radius: 10px; color: #e2e8f0; font-size: 14px; font-weight: 700; cursor: pointer; text-align: center; font-family: 'JetBrains Mono', monospace; }
  .api-choice:hover:not(:disabled) { border-color: #10b981; }
  .api-choice:disabled { cursor: default; }
  .api-correct { border-color: #10b981 !important; background: rgba(16,185,129,0.08); }
  .api-wrong { border-color: #ef4444 !important; background: rgba(239,68,68,0.08); }
