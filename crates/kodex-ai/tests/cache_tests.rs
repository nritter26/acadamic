...
fn test_cache_set_and_get() {
    ...
    cache.set(&AiProvider::Keyword, "test-model", "[{\"role\":\"user\"}]", "hello");
...
fn test_cache_expiry() {
    ...
    cache.set(&AiProvider::Keyword, "m", "[{}]", "data");
    assert_eq!(cache.len(), 1);
    std::thread::sleep(Duration::from_millis(50));
...
fn test_cache_clear() {
    let cache = LLMCache::with_config(10, Duration::from_secs(60));
    cache.set(&AiProvider::Keyword, "m", "[{}]", "a");
    cache.set(&AiProvider::OpenAI, "m", "[{}]", "b");
...
fn test_cache_lru_eviction() {
    ...
    cache.set(&AiProvider::Keyword, "m", "[1]", "a");
    cache.set(&AiProvider::Keyword, "m", "[2]", "b");
    cache.set(&AiProvider::Keyword, "m", "[3]", "c");
    assert_eq!(cache.len(), 2);
...
fn test_cache_different_providers() {
    let cache = LLMCache::with_config(10, Duration::from_secs(60));
    cache.set(&AiProvider::OpenAI, "gpt-4", "[{}]", "openai-response");
    cache.set(&AiProvider::Anthropic, "claude-3", "[{}]", "anthropic-response");
...
fn test_cache_different_models() {
    let cache = LLMCache::with_config(10, Duration::from_secs(60));
    cache.set(&AiProvider::OpenAI, "gpt-4", "[{}]", "gpt4");
    cache.set(&AiProvider::OpenAI, "gpt-3.5", "[{}]", "gpt35");
