...
export function analyzeCode(code: string, lang?: string): AnalysisResult {
  ...
  if (lang === 'js') {
    for (const [pattern, hint] of Object.entries(COMMON_PATTERNS)) {
      if (code.includes(pattern)) hints.push(hint);
    }
    ...
    if (parenDiff !== 0) hints.push('Unbalanced parentheses! Make sure every `(` has a matching `)`.');

    const braceDiff = (code.match(/\{/g) || []).length - (code.match(/\}/g) || []).length;
    if (braceDiff !== 0) hints.push('Unbalanced curly braces! Make sure every `{` has a matching `}`.');

    const bracketDiff = (code.match(/\[/g) || []).length - (code.match(/\]/g) || []).length;
    if (bracketDiff !== 0) hints.push('Unbalanced square brackets! Make sure every `[` has a matching `]`.');
  } else if (lang === 'ts') {
    if (/\bany\b/.test(code)) hints.push('Avoid `any` when possible. Prefer `unknown`, unions, or a concrete interface.');
    if (/\s+as\s+[A-Za-z_][\w<>\[\]\|?]*/.test(code)) hints.push('Type assertions (`as`) bypass safety checks. Narrow the type if you can.');
    if (/\w+!\b/.test(code)) hints.push('Non-null assertions (`!`) can hide bugs. Prefer a null check or optional chaining.');
  } else if (lang === 'py') {
    ...
    if (tabs && spaces) hints.push('Mixing tabs and spaces in indentation causes hard-to-debug errors. Stick to 4 spaces.');
    if (/^\s*except\s*:/m.test(code)) hints.push('Avoid bare `except:` blocks. Catch a specific exception when you know what can fail.');
  } else if (lang === 'go') {
...
export function analyzeUserCode(code: string, lang: string): string[] | null {
  ...
  if (lang === 'js') {
    ...
    if (unclosedBraces > 0) hints.push(`You have ${unclosedBraces} unclosed curly braces.`);
    if (unclosedBraces < 0) hints.push(`You have ${Math.abs(unclosedBraces)} too many closing braces.`);
    if (unclosedParens > 0) hints.push(`You have ${unclosedParens} unclosed parentheses.`);
    ...
    if (code.includes('==')) hints.push('Consider using `===` (strict equality) instead of `==`.');
    if (code.includes('var ')) hints.push('Use `let` or `const` instead of `var`.');
  } else if (lang === 'py') {
    ...
    if (/\bany\b/.test(code)) hints.push('Replace `any` with `unknown`, unions, or a concrete interface when possible.');
    if (/\s+as\s+[A-Za-z_][\w<>\[\]\|?]*/.test(code)) hints.push('Type assertions can bypass safety checks. Prefer narrowing or a type guard.');
    if (/\w+!\b/.test(code)) hints.push('Non-null assertions can hide bugs. Prefer a null check or optional chaining.');
  } else if (lang === 'go') {
    if (/\berr\b/.test(code) && !/if\s+\w+\s*!=\s*nil/.test(code)) {
