...
impl AppConfig {
    pub fn from_env() -> Self {
        ...
        // Resolve project root from crate path at compile time so paths work from any CWD
        let project_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .ancestors()
            .nth(4) // kodex-core crate -> crates/ -> rust/ -> backend/ -> Kodex (project root)
            .map(|p| p.to_path_buf())
            .unwrap_or_else(|| PathBuf::from("."));

        let data_dir = PathBuf::from(
            env::var("DATA_DIR").unwrap_or_else(|_| {
                project_root.join("data").to_string_lossy().to_string()
            })
        );
