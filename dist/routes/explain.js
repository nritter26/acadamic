...
function analyzeCode(code) {
    ...
    while ((m = callRe.exec(code)) !== null) {
        const fullCall = m[1];
        if (fullCall === 'if' || fullCall === 'while' || fullCall === 'for' || fullCall === 'function')
            continue;
    ...
    if (/\bimport\s/.test(code) || /\brequire\s*\(/.test(code))
        result.imports.push('module imports');
    if (/\btry\b/.test(code) && /\bcatch\b/.test(code))
        result.patterns.push('error handling');
    if (/\basync\b|\bawait\b/.test(code))
        result.patterns.push('async/await');
    if (/\bPromise\b/.test(code))
        result.patterns.push('Promises');
    if (/\.map\s*\(/.test(code))
        result.patterns.push('Array.map');
    if (/\.filter\s*\(/.test(code))
        result.patterns.push('Array.filter');
    if (/\.reduce\s*\(/.test(code))
        result.patterns.push('Array.reduce');
    if (/\.then\s*\(/.test(code))
        result.patterns.push('Promise chaining');
    if (/console\.log\s*\(/.test(code))
        result.flow.push('prints to console');
    if (/fetch\s*\(/.test(code))
        result.flow.push('makes HTTP requests');
    if (/addEventListener|\.on\w+\s*=/.test(code))
        result.flow.push('handles events');
    result.patterns = [...new Set(result.patterns)];
    return result;
}
function generateStaticExplain(code, lang, topic) {
