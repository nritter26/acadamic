...
function safeLangParam(lang) {
    ...
    if (!match)
        throw new AppError(404, `Content file '${safe}' not found`);
    return match;
}
...
router.get('/:lang/:phase', (req, res) => {
    ...
    try {
        ...
        if (!data[phase])
            throw new AppError(404, `Phase '${phase}' not found in ${file.replace('.json', '')}`);
...
router.put('/:lang', requireAuth, validate(UpdateContentSchema), (req, res) => {
    const file = safeLangParam(req.params.lang);
    const filePath = path.join(CONTENT_DIR, file);
    const { data } = req.body;
    // Validate structure
...
router.delete('/:lang', requireAuth, (req, res) => {
    const file = safeLangParam(req.params.lang);
    const filePath = path.join(CONTENT_DIR, file);
    const backupPath = filePath + '.bak';
    try {
