...
export function detectLangFromMsg(msg) {
    ...
    if (/\b(python|py\b)/.test(lower) && !/\b(py\s+script|pypy)\b/.test(lower))
        return 'py';
    if (/\b(javascript|js\b)/.test(lower) && !/\b(jsx|json)\b/.test(lower))
        return 'js';
    if (/\b(typescript|ts\b)/.test(lower))
        return 'ts';
    if (/\b(golang|go\s+lang)\b/.test(lower) || (/\bgo\b/.test(lower) && /\b(goroutine|gopath|gofmt|package|import|func\s+main)\b/.test(lower)))
        return 'go';
    if (/\brust\b/.test(lower) && !/\brusty\b|rust[- ]proof|surface\s+rust/.test(lower))
        return 'rs';
    if (/\b(c\s*(\+\+|plus\s*plus)|cpp)\b/.test(lower))
        return 'cpp';
    if (/\b(c\s*sharp|csharp)\b/.test(lower))
        return 'cs';
    if (/\bkotlin\b/.test(lower))
        return 'kt';
    if (/\bscala\b/.test(lower))
        return 'scala';
    if (/\bswift\b/.test(lower))
        return 'swift';
    if (/\bzig\b/.test(lower))
        return 'zig';
    if (/\bc\b/.test(lower) && /\b(pointer|malloc|free|printf|scanf|struct|union|sizeof)\b/.test(lower))
        return 'c';
    return null;
}
