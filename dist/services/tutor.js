...
function resolveFollowUp(q, history) {
    ...
    if (lang) {
        const rawName = LANG_NAMES[lang] || lang;
...
export async function buildLLMMessages(message, lang, topic, phase, code, output, hasError, history, learnerId) {
    const messages = [];
    const context = [];
    ...
    if (learnerId && lang) {
        try {
            ...
            logger.debug({ error: String(e) }, 'Learner profile failed');
        }
    }
...
export async function handleTutorMessage(message, options, sseSend, sseDone) {
