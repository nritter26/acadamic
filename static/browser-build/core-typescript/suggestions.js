...
function getDynamicSuggestions(outputText, _conversationHistory, currentTopic, _streamingFullText) {
    ...
    if (currentTopic) {
        ...
        for (const [key, hints] of Object.entries(topHints)) {
            if (topicLC.includes(key.toLowerCase()))
                return hints;
        }
...
function updateAISuggestions(currentLang, currentTopic, _streamingFullText) {
    ...
    if (streamingFullText) {
        ...
        if (text.includes('variable') || text.includes('declare'))
            followUps.push('Show me a variable example');
        if (text.includes('function') || text.includes('method'))
            followUps.push('Give me a function exercise');
        if (text.includes('loop') || text.includes('for ') || text.includes('while'))
            followUps.push('Show me a loop example');
        if (text.includes('class') || text.includes('object'))
            followUps.push('Practice: build a class');
        if (text.includes('array') || text.includes('list'))
            followUps.push('Practice with arrays');
        if (text.includes('error') || text.includes('debug'))
            followUps.push('How do I debug this?', 'Common mistakes');
        if (followUps.length > 0) {
            followUps.push('Tell me more', 'Give me an example');
            const buttons = followUps.slice(0, 4).map(s => `<button onclick="askAI('${s.replace(/'/g, "\\'")}')">${s}</button>`);
    ...
    if (hasAI) {
        ...
        if (hasExercise)
            buttons.push(`<button onclick="generateExercise()" style="background:#0ea5e9;color:#000;">✨ Exercise</button>`);
        if (currentTopic)
            buttons.push(`<button onclick="generateQuiz()" style="background:#f59e0b;color:#000;">📝 Quiz</button>`);
