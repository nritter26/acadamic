...
function answerQuiz(qIdx, optIdx) {
    ...
    if (quizRoundDone)
        return;
    if (qIdx >= quizRoundQuestions.length)
        return;
    const globalIdx = quizRoundQuestions[qIdx];
    if (quizAnswers[globalIdx] !== undefined)
        return;
    quizAnswers[globalIdx] = optIdx;
    quizScore.total++;
    if (optIdx === questions[globalIdx].ans)
        quizScore.correct++;
...
function renderChallengeList() {
    ...
    challenges.forEach((ch, i) => {
        if (currentLevel !== 'all' && ch.level !== currentLevel)
            return;
        if (challengeSearchQuery && !ch.title.toLowerCase().includes(challengeSearchQuery))
            return;
...
function showHint() {
    ...
    if (!ch._diff)
        ch._diff = computeDiff(ch.bug, ch.solution);
    hintLevel++;
    if (hintLevel > 3)
        hintLevel = 3;
    ...
    if (hintLevel === 1) {
        ...
        if (btn) {
            ...
            btn.disabled = true;
            btn.className = 'challenge-btn primary';
        }
        out.innerHTML = html;
        return;
    }
    ...
    out.innerHTML = html;
    if (btn)
        btn.className = 'challenge-btn hint-active';
}
...
function testChallenge() {
    ...
    if (!ch) {
        setRunLoading(false);
        document.getElementById('output').innerText = '// No challenge selected';
        return;
    }
    ...
    if (challengeLang === 'js') {
        ...
        try {
            ...
            let html = '';
            if (captured)
                html += '<pre style="font-size:10px;color:#94a3b8;margin:0 0 8px 0;">' + escapeHtml(captured) + '</pre>';
...
document.getElementById('editor').addEventListener('keydown', function (e) {
    ...
    if ((key === 'ArrowDown' || key === 'ArrowUp') && compState && compState.popup.style.display !== 'none') {
        ...
        if (key === 'ArrowDown')
            compState.idx = (compState.idx + 1) % items.length;
        else
            compState.idx = (compState.idx - 1 + items.length) % items.length;
...
document.getElementById('editor').addEventListener('keyup', function (e) {
    if (['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Home', 'End', 'PageUp', 'PageDown', 'Control', 'Shift', 'Alt', 'Meta', 'Escape'].includes(e.key))
        return;
