...
setMode = function (lang) {
    ...
    if (platformBar)
        platformBar.style.display = 'none';
    ...
    if (lang === 'tutorial') {
        ...
        if (tutorialNavBtn)
            tutorialNavBtn.classList.add('active');
    ...
    if (runBtn)
        runBtn.textContent = lang === 'challenge' ? 'Test ▶' : 'Run ▶';
    const apiBtn = document.getElementById('api-toggle-btn');
    if (apiBtn)
        apiBtn.style.display = 'none';
    if (lang === 'quiz') {
    ...
    phases.forEach((phase, i) => {
        if (i < third)
            phaseLevels[phase] = 'beginner';
        else if (i < third * 2)
            phaseLevels[phase] = 'intermediate';
        else
            phaseLevels[phase] = 'expert';
    });
    if (levelBar)
        renderLevelBar();
    if (lang === 'gamedev') {
...
function renderTopicList(lang, prefixHtml) {
    ...
    for (const phase in langData) {
        ...
        for (const t of topics) {
            if (completedTopics.has(currentLang + ':' + t))
                phaseDone++;
        }
...
function initLineNumbers() {
    ...
    if (!wrapper)
        return;
    if (wrapper.querySelector('.editor-lines'))
        return;
...
function toggleRoadmapView() {
    ...
    if (!wasOpen) {
        ...
        if (title)
            title.textContent = langName.charAt(0).toUpperCase() + langName.slice(1) + ' Roadmap';
        roadmapRendered = false;
...
function renderRoadmap(container, lang) {
    ...
    if (titleEl)
        titleEl.textContent = langName + ' Roadmap';
    const phases = Object.keys(langData);
...
(function gtranslateInit() {
    ...
    if (ed)
        ed.classList.add('notranslate');
    if (out)
        out.classList.add('notranslate');
