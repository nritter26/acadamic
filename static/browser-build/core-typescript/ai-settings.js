...
function switchConfigForProvider(provider) {
...
function renderSettings(container) {
  ...
  `;
    const select = container.querySelector('#aiProviderSelect');
    const modelInput = container.querySelector('#aiModelInput');
    const endpointInput = container.querySelector('#aiEndpointInput');
    const keyInput = container.querySelector('#aiKeyInput');
    const modelRow = container.querySelector('#aiModelRow');
    const endpointRow = container.querySelector('#aiEndpointRow');
    const keyRow = container.querySelector('#aiKeyRow');
    const refreshBtn = container.querySelector('#aiRefreshModels');
    const ollamaRow = container.querySelector('#aiOllamaModelsRow');
    const ollamaList = container.querySelector('#aiOllamaModelsList');
    const updateRows = () => {
    ...
    async function fetchOllamaModels() {
        ...
        try {
            ...
            ollamaList.innerHTML = models.map(m => `<button class="ai-ollama-model-btn" data-model="${m}">${m}</button>`).join('');
