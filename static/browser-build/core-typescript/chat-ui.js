...
function highlightAICode(code, lang) {
    ...
    return lines.map(line => {
        ...
        while (i < line.length) {
            ...
            if (sCm) {
                tokens.push('<span class="syn-comment">' + sCm[0] + '</span>');
                i += sCm[0].length;
                continue;
            }
            const bCm = rest.match(/^\/\*[\s\S]*?\*\//);
            if (bCm) {
                tokens.push('<span class="syn-comment">' + bCm[0] + '</span>');
                i += bCm[0].length;
                continue;
            }
            const str = rest.match(/^("(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'|`(?:[^`\\]|\\.)*`)/);
            if (str) {
                tokens.push('<span class="syn-string">' + str[1] + '</span>');
                i += str[1].length;
                continue;
            }
            const num = rest.match(/^\b(\d+\.?\d*|0x[0-9a-fA-F]+)\b/);
            if (num) {
                tokens.push('<span class="syn-number">' + num[1] + '</span>');
                i += num[1].length;
                continue;
            }
            const word = rest.match(/^([a-zA-Z_$][\w$]*)/);
            if (word) {
                if (keywords.includes(word[1]))
                    tokens.push('<span class="syn-keyword">' + word[1] + '</span>');
                else
                    tokens.push(word[1]);
                i += word[1].length;
                continue;
...
function inlineFormat(text, codeBlocks) {
    ...
    t = t.replace(/\x00CODEBLOCK(\d+)\x00/g, (_match, idx) => {
        const cb = codeBlocks[parseInt(idx)];
        if (cb)
            return `<div class="ai-code-wrapper"><pre class="ai-code-block"><code>${cb.highlighted}</code></pre><button class="ai-run-code" id="ai-code-${++aiCodeId}" data-code="${cb.safeCode}">Run</button></div>`;
        return '';
    });
    return t;
}
function formatAIText(text) {
    ...
    for (let li = 0; li < lines.length; li++) {
        ...
        if (isListItem) {
            ...
            if (!inList) {
                result += `<${tag} style="margin:4px 0;padding-left:20px;">`;
                inList = true;
                listStack.push(tag);
            }
